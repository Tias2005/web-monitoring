  ...
  return (
    <div className="detail-panel">
      <div className="detail-content">
        ...
        {isLembur ? (
          <>
            <div className="detail-row"><span>Tanggal Lembur</span><strong>
              {detail.tanggal_mulai.split(' ')[0]}</strong></div>
            <div className="detail-row"><span>Jam Mulai</span><strong className="text-blue">
              {detail.jam_mulai || '--:--'}</strong></div>
            <div className="detail-row"><span>Jam Selesai</span><strong className="text-blue">
              {detail.jam_selesai || '--:--'}</strong></div>
          </>
        ) : (
          <>
            <div className="detail-row"><span>Tanggal Mulai</span><strong>
              {detail.tanggal_mulai.split(' ')[0]}</strong></div>
            <div className="detail-row"><span>Tanggal Selesai</span><strong>
              {detail.tanggal_selesai.split(' ')[0]}</strong></div>
          </>
        )}
        
        <div className="detail-row"><span>Alasan</span><strong>
          {detail.alasan}</strong></div>
